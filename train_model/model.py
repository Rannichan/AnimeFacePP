...
def cls_ohem(cls_prob, label):
    ...
    :param cls_prob: float tensor, (batch_size, 2)
    :param label: int tensor, (batch_size, )
    :return:
    """
    zeros = tf.zeros_like(label)  # (batch_size, )
    #pos 1 -> 1, neg 0 -> 0, part -1 -> 0, landmark -2 -> 0
    label_filter_invalid = tf.where(tf.less(label,0), zeros, label)  # (batch_size, )
    num_cls_prob = tf.size(cls_prob)  # 2 * batch_size
    cls_prob_reshape = tf.reshape(cls_prob, [num_cls_prob,-1])  # （2 * batch_size, 1）
    label_int = tf.cast(label_filter_invalid, tf.int32)
    num_row = tf.to_int32(cls_prob.get_shape()[0])  # batch_size
    row = tf.range(num_row) * 2  # (batch_size, ), (0, 2, 4, ...)
    indices_ = row + label_int  # pos +1, else +0
    label_prob = tf.squeeze(tf.gather(cls_prob_reshape, indices_))  # (batch_size, )
    loss = -tf.log(label_prob + 1e-10)  # (batch_size, )
    zeros = tf.zeros_like(label_prob, dtype=tf.float32)  # (batch_size, )
    ones = tf.ones_like(label_prob, dtype=tf.float32)  # (batch_size, )
    # pos 1 -> 1, neg 0 -> 1, part -1 -> 0, landmark -2 -> 0
...
def Manga_Net(inputs, label=None, bbox_target=None):
    with slim.arg_scope([slim.conv2d],
        ...
        cls_prob = slim.fully_connected(fc1, num_outputs=2, scope="cls_fc", activation_fn=None)
        cls_pred = tf.math.argmax(cls_prob, axis=1)
        bbox_pred = slim.fully_connected(fc1, num_outputs=4, scope="bbox_fc", activation_fn=None)

        cls_loss = tf.losses.sparse_softmax_cross_entropy(label, cls_prob)
        bbox_loss = bbox_ohem(bbox_pred, bbox_target, label)
        L2_loss = tf.add_n(slim.losses.get_regularization_losses())
        # print(cls_prob.name, cls_pred.name, bbox_pred.name, cls_loss.name, bbox_loss.name, L2_loss.name)
        # cls_fc/BiasAdd:0 ArgMax:0 bbox_fc/BiasAdd:0 sparse_softmax_cross_entropy_loss/value:0 Mean:0 AddN:0

        return cls_loss, bbox_loss, L2_loss, cls_pred, bbox_pred
